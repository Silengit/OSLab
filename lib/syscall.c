...
int32_t syscall(uint32_t eax, uint32_t ecx)
{
	int32_t ret = 0;

	//内嵌汇编 保存 eax, ecx 至通用寄存器

	asm volatile("int $0x80": "=a"(ret):"a"(eax),"c"(ecx));
		
	return ret;//what will ret be like?
}

void printch(char c){
	syscall(SYS_write, (uint32_t)&c);
}
...
void printint(int x){
	...
	if(x == 0){
		str = "0";
		syscall(SYS_write, (uint32_t)&str[0]);
		return;
	}
	if(x == 0x80000000){
		str = "-2147483648";
		while(str[index]!='\0')
			syscall(SYS_write, (uint32_t)&str[index++]);
		return;
	}
	if(x < 0){
		buf[0] = '-';
		syscall(SYS_write, (uint32_t)&buf[0]);
		x = -x;
	}
	...
	buf[len] = '\0';
	while(buf[index]!='\0')
		syscall(SYS_write, (uint32_t)&buf[index++]);
}
